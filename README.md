...
See [the 32-skill audit](SKILL-AUDIT.md) for necessity, scope, and validation limits.
...
| 29 | [remote-host-access](remote-host-access/SKILL.md) | Diagnose DNS, routes, firewalls, listening sockets, activation, and auth from evidence. Refusal may be a listener or firewall REJECT. Authorized repairs preserve management access and narrow exposure. |
...
| 27 | [delegate-run](delegate-run/SKILL.md) | Finish authorized work with minimal supervision: acceptance checks, resumable state, existing authorization, useful progress updates, and Astra’s final verification. Uses orchestration when independent work benefits from workers. |
